 ...
 * @Last Modified time: 2018-08-14 09:41:52
 */
<template>
  <section>
    <el-row class="transparent mt-3">
      <el-col class="p-3 mt-3" :span="24" v-loading="visible.alarmUnitLoading">
        <perspective-nav-button :alarm-unit="alarmUnit" @alarmUnit="getAlarmUnit"></perspective-nav-button>
      </el-col>
      <!-- 搜索 start -->
      <el-col class="toolbar text-right" :span="24">
        <el-form :inline="true" :model="filters" @submit.native.prevent>
          <el-form-item>
            <el-select v-model="filters.deviceTypeId" placeholder="请选择设备种类" @change="initAlarmUnitData">
          ...
          <el-form-item>
            <el-select v-model="filters.warnCategoryId" placeholder="设备所属报警种类" @change="initAlarmUnitData">
              <el-option
        ...
        <el-table highlight-current-row :data="alarmUnitData" style="width: 100%">
          ...
          <el-table-column label="选择参数" width="200">
            <template slot-scope="scope">
              <el-button type="text" size="medium" icon="" @click="openMesWorkstage(scope.row)">点击选择</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-col>
...
<script>
  ...
  import { queryConfDevice, queryWarnUnit, saveModuleUnitThree } from '../../../api/api'
  ...
  export default {
    ...
    computed: {
      ...mapGetters(['getDeviceTypeList', 'getAlarmTypeList']),
      alarmTypeList () {
        // 去掉离线报警、停机报警
        let newData = []
    ...
    methods: {
      ...
      initAlarmUnitData () {
        ...
        this.filters.unitTypeId = this.perspectiveId
    ...
    watch: {
      apllySave (val) {
        if (val) {
          ...
          this.$emit('save', _p)
          this.$emit('saveData', this.perspectiveId)
        }
      }
    }
  }
