/*
 * @Author: liucz 
 * @Date: 2018-06-26 17:13:38 
 * @Last Modified by: liucz
 * @Last Modified time: 2018-08-08 14:17:20
 */
<template>
  <section>
    <!-- 操作提示组件 -->
    <operation-tips video="">
      <span class="color-999">配置全系统的报警等级参数。报警分为5个等级，每个设备的每个参数可以配置每个报警等级的上下范围值。</span>
    </operation-tips>
    ...
    <el-table :data="list" highlight-current-row v-loading="visible.listLoading">
      ...
      <el-table-column prop="status_config_parameter" label="配置状态" sortable>
        <template slot-scope="scope">
          <span :class="{'color-active': scope.row.status_config_parameter === '1'}">
            {{ scope.row.status_config_parameter === '1' ? '已配置' : '未配置' }}
          </span>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="" label="操作">
        <template slot-scope="scope">
          <el-button type="text" size="medium" @click="toConfig(scope.row)">配置参数报警等级</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!--列表 end-->
  ...
  export default {
    ...
    methods: {
      ...
      toConfig (row) {
        ...
        this.$router.push({
          path: `/sysConfig/deviceAlarmLevel/${row.device_id}`,
          query: {
            name: row.deviceName,
