 ...
 * @Last Modified time: 2018-08-21 15:51:33
 */
<template>
  <el-dialog :title="title || '选择预警单元' " :visible.sync="visible" class="mes-dialog" width="40%">
    ...
    <el-table :data="list" v-loading="listLoading"
      ...
      <el-table-column prop="warning_unit_name" label="车间名称" sortable></el-table-column>
...
<script>
  ...
  import { queryWarnUnit2 } from '../../../api/api'
  export default {
    name: 'warning-unit',
    data () {
      return {
    ...
    methods: {
      ...
      handleRowSelect (rows, row) {
        let checked = false
        rows.map(v => v.warning_uint_id).forEach(id => {
          if (id === row.warning_uint_id) {
            checked = true
          }
        })
        // 选中 - 新增一行
      ...
      checkedSelectRows (checkedRows, rows, type = 'add') {
        // 新增
        if (type === 'add') {
          this.selectedIds = [...new Set([...this.selectedIds, ...rows.map(v => v.warning_uint_id)])]
          ...
          // 删除
          rows.forEach(row => {
            let index = this.selectedIds.indexOf(row.warning_uint_id)
            if (index > -1) {
      ...
      confirmSelected (row) {
        ...
        if (this.multiple) {
          ...
          this.selectedIds.forEach((id, idx) => {
            this.selectedRows[idx] = { warning_uint_id: id }
          })
        } else {
          // 选择单行
      ...
      async getList () {
        ...
        let res = await queryWarnUnit2(para)
        if (res.status === 0) {
          this.filters.total = res.map.line
          this.list = res.map.warnUnit
      ...
      checkSelectedIds () {
        ...
        if (this.selectedIds.length > 0 && this.multiple) {
          ...
          this.list.forEach((item, index) => {
            if (item.warning_uint_id === this.selectedIds[0]) {
              this.$nextTick(() => {
