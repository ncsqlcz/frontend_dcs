...
const WarningUnit = function (options, callback) {
  ...
  return new Promise((resolve, reject) => { // eslint-disable-line
    currentMsg = {
      options: Object.assign({}, defaults, WarningUnit.defaults, options),
      callback: callback,
      resolve: resolve,
      reject: reject
    }
...
// 设置默认配置
WarningUnit.setDefaults = defaults => {
  WarningUnit.defaults = defaults
}

// 显示弹框
WarningUnit.show = (title, options) => {
  ...
  return WarningUnit(Object.assign({
    title: title
  }, options))
}
// 手动关闭弹框
