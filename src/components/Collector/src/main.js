...
const Collector = function (options, callback) {
  ...
  return new Promise((resolve, reject) => { // eslint-disable-line
    currentMsg = {
      options: Object.assign({}, defaults, Collector.defaults, options),
      callback: callback,
      resolve: resolve,
      reject: reject
    }
...
// 设置默认配置
Collector.setDefaults = defaults => {
  Collector.defaults = defaults
}

// 显示弹框
Collector.show = (title, options) => {
  ...
  return Collector(Object.assign({
    title: title
  }, options))
}
// 手动关闭弹框
