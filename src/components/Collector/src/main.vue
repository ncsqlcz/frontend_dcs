/*
 * @Author: liucz 
 * @Date: 2018-06-25 16:53:45 
 * @Last Modified by: liucz
 * @Last Modified time: 2018-08-01 15:11:01
 */
<template>
  <el-dialog :title="title || '选择设备' " :visible.sync="visible" class="mes-dialog" width="40%">
    ...
    <el-col>
       <el-col :span="12" class="color-999" :offset="1">只能与一台智能物联网关建立冗余关系</el-col>
    ...
    <el-table :data="list" v-loading="listLoading"
      ...
      <el-table-column prop="collector_ip" label="智能物联网关IP" sortable></el-table-column>
...
<script>
  ...
  import { queryCollectorBackup } from '../../../api/api'
  export default {
    name: 'device',
    data () {
      return {
    ...
    methods: {
      ...
      handleRowSelect (rows, row) {
        let checked = false
        rows.map(v => v.collector_id).forEach(id => {
          if (id === row.collector_id) {
            checked = true
          }
        })
        // 选中 - 新增一行
      ...
      checkedSelectRows (checkedRows, rows, type = 'add') {
        // 新增
        if (type === 'add') {
          this.selectedIds = [...new Set([...this.selectedIds, ...rows.map(v => v.collector_id)])]
          ...
          // 删除
          rows.forEach(row => {
            let index = this.selectedIds.indexOf(row.collector_id)
            if (index > -1) {
      ...
      confirmSelected (row) {
        ...
        if (this.multiple) {
          ...
          this.selectedIds.forEach((id, idx) => {
            this.selectedRows[idx] = { collector_id: id }
          })
        } else {
          // 选择单行
      ...
      getDataList () {
        ...
        queryCollectorBackup(para).then((data) => {
          if (data.status === 0) {
            this.filters.total = data.map.CollectorBackup.length
            this.list = data.map.CollectorBackup
      ...
      checkSelectedIds () {
        ...
        if (this.selectedIds.length > 0 && this.multiple) {
          ...
          this.list.forEach((item, index) => {
            if (item.collector_id === this.selectedIds[0]) {
              this.$nextTick(() => {
