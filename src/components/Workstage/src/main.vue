 ...
 * @Last Modified time: 2018-07-31 14:29:04
 */
<template>
  ...
  export default {
    ...
    methods: {
      ...
      handleRowSelect (rows, row) {
        let checked = false
        rows.map(v => v.process_id).forEach(id => {
          if (id === row.process_id) {
            checked = true
          }
        })
        // 选中 - 新增一行
      ...
      checkedSelectRows (checkedRows, rows, type = 'add') {
        // 新增
        if (type === 'add') {
          this.selectedIds = [...new Set([...this.selectedIds, ...rows.map(v => v.process_id)])]
          ...
          // 删除
          rows.forEach(row => {
            let index = this.selectedIds.indexOf(row.process_id)
            if (index > -1) {
      ...
      confirmSelected (row) {
        ...
        if (this.multiple) {
          ...
          //   this.selectedRows[idx] = { process_id: id }
          // })
        } else {
          // 选择单行
      ...
      getDataList () {
        ...
        queryWorkShopOrProcess(para).then((data) => {
          if (data.status === 0) {
            ...
            this.list = data.map.processDataMap
      ...
      checkSelectedIds () {
        ...
        if (this.selectedIds.length > 0 && this.multiple) {
          ...
          this.list.forEach((item, index) => {
            if (item.process_id === this.selectedIds[0]) {
              this.$nextTick(() => {
