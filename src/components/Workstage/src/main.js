...
const Workstage = function (options, callback) {
  ...
  return new Promise((resolve, reject) => { // eslint-disable-line
    currentMsg = {
      options: Object.assign({}, defaults, Workstage.defaults, options),
      callback: callback,
      resolve: resolve,
      reject: reject
    }
...
// 设置默认配置
Workstage.setDefaults = defaults => {
  Workstage.defaults = defaults
}

// 显示弹框
Workstage.show = (title, options) => {
  ...
  return Workstage(Object.assign({
    title: title
  }, options))
}
// 手动关闭弹框
