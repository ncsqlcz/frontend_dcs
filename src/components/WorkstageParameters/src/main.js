...
const WorkstageParameters = function (options, callback) {
  ...
  return new Promise((resolve, reject) => { // eslint-disable-line
    currentMsg = {
      options: Object.assign({}, defaults, WorkstageParameters.defaults, options),
      callback: callback,
      resolve: resolve,
      reject: reject
    }
...
// 设置默认配置
WorkstageParameters.setDefaults = defaults => {
  WorkstageParameters.defaults = defaults
}

// 显示弹框
WorkstageParameters.show = (title, options) => {
  ...
  return WorkstageParameters(Object.assign({
    title: title
  }, options))
}
// 手动关闭弹框
WorkstageParameters.close = () => {
  this.action = ''
  this.visible = false
}

export default WorkstageParameters
export { WorkstageParameters }
