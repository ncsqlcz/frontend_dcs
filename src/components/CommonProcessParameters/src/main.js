...
const ProcessParameters = function (options, callback) {
  ...
  return new Promise((resolve, reject) => { // eslint-disable-line
    currentMsg = {
      options: Object.assign({}, defaults, ProcessParameters.defaults, options),
      callback: callback,
      resolve: resolve,
      reject: reject
    }
...
// 设置默认配置
ProcessParameters.setDefaults = defaults => {
  ProcessParameters.defaults = defaults
}

// 显示弹框
ProcessParameters.show = (title, options) => {
  ...
  return ProcessParameters(Object.assign({
    title: title
  }, options))
}
// 手动关闭弹框
ProcessParameters.close = () => {
  this.action = ''
  this.visible = false
}
