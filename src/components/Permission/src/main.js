...
const Permission = function (options, callback) {
  ...
  return new Promise((resolve, reject) => { // eslint-disable-line
    currentMsg = {
      options: Object.assign({}, defaults, Permission.defaults, options),
      callback: callback,
      resolve: resolve,
      reject: reject
    }
...
// 设置默认配置
Permission.setDefaults = defaults => {
  Permission.defaults = defaults
}

// 显示弹框
Permission.show = (title, options) => {
  ...
  return Permission(Object.assign({
    title: title
  }, options))
}
// 手动关闭弹框
