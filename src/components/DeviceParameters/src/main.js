...
const DeviceParameters = function (options, callback) {
  ...
  return new Promise((resolve, reject) => { // eslint-disable-line
    currentMsg = {
      options: Object.assign({}, defaults, DeviceParameters.defaults, options),
      callback: callback,
      resolve: resolve,
      reject: reject
    }
...
// 设置默认配置
DeviceParameters.setDefaults = defaults => {
  DeviceParameters.defaults = defaults
}

// 显示弹框
DeviceParameters.show = (title, options) => {
  ...
  return DeviceParameters(Object.assign({
    title: title
  }, options))
}
// 手动关闭弹框
DeviceParameters.close = () => {
  this.action = ''
  this.visible = false
}
