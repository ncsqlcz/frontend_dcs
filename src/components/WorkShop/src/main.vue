/*
 * @Author: liucz 
 * @Date: 2018-06-29 14:44:52 
 * @Last Modified by: liucz
 * @Last Modified time: 2018-07-31 14:19:19
 */
<template>
  <el-dialog :title="title || '选择车间' " :visible.sync="visible" class="mes-dialog" width="40%">
    ...
    <el-table :data="list" v-loading="listLoading"
              ...
              ref="multipleTable"
              @__select="handleRowSelect"
              @__select-all="handleSelectAll"
      ...
      <el-table-column prop="workshop_name" label="车间名称" sortable></el-table-column>
...
<script>
  ...
  import { queryWorkShopOrProcess } from '../../../api/api'
  export default {
    name: 'work-shop',
    data () {
      return {
    ...
    methods: {
      ...
      handleRowSelect (rows, row) {
        let checked = false
        rows.map(v => v.workshop_id).forEach(id => {
          if (id === row.workshop_id) {
            checked = true
          }
        })
        // 选中 - 新增一行
      ...
      checkedSelectRows (checkedRows, rows, type = 'add') {
        // 新增
        if (type === 'add') {
          this.selectedIds = [...new Set([...this.selectedIds, ...rows.map(v => v.workshop_id)])]
          ...
          // 删除
          rows.forEach(row => {
            let index = this.selectedIds.indexOf(row.workshop_id)
            if (index > -1) {
      ...
      confirmSelected (row) {
        ...
        if (this.multiple) {
          ...
          //   this.selectedRows[idx] = { workshop_id: id }
          // })
        } else {
          // 选择单行
      ...
      getDataList () {
        ...
        queryWorkShopOrProcess(para).then((data) => {
          if (data.status === 0) {
            this.filters.total = data.map.line
            this.list = data.map.workShopDataMap
      ...
      checkSelectedIds () {
        ...
        if (this.selectedIds.length > 0 && this.multiple) {
          ...
          this.list.forEach((item, index) => {
            if (item.workshop_id === this.selectedIds[0]) {
              this.$nextTick(() => {
